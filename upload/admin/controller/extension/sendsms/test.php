<?php
class ControllerExtensionSendSMSTest extends Controller
{
    private $error = array();
    ...
    public function index()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $this->model_extension_sendsms_send->send_sms($this->request->post['module_sendsms_phone'], $this->request->post['module_sendsms_message'], $this->request->post['module_sendsms_short'], $this->request->post['module_sendsms_gdpr'], 'test');

            $this->session->data['success'] = $this->language->get('text_success_test_send');

            $this->response->redirect($this->url->link('extension/sendsms/test', 'user_token=' . $this->session->data['user_token'], true));
        }
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/sendsms/test', 'user_token=' . $this->session->data['user_token'], true)
        );
        ...
        $data['action'] = $this->url->link('extension/sendsms/test', 'user_token=' . $this->session->data['user_token'], true);
        ...
        $data['entry_gdpr'] = $this->language->get('entry_gdpr');
        $data['entry_short'] = $this->language->get('entry_short');
        $data['help_gdpr'] = $this->language->get('help_gdpr');
        $data['help_short'] = $this->language->get('help_short');
        $data['aprox_nr_msg'] = $this->language->get('aprox_nr_msg');
        ...
        $this->response->setOutput($this->load->view('extension/sendsms/test', $data));
    }
